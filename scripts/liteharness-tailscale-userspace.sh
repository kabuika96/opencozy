...
uid="$(id -u)"
domain="gui/$uid"
label="com.liteharness.tailscaled.userspace"
script_dir="${0:A:h}"
...
log_dir="$(configured_value "LITEHARNESS_TAILSCALE_LOG_DIR" "$home_dir/Library/Logs/LiteHarness")"
state_dir="$(configured_value "LITEHARNESS_TAILSCALE_STATE_DIR" "$home_dir/.local/share/liteharness-tailscale")"
socket_path="$(configured_value "LITEHARNESS_TAILSCALE_SOCKET" "$state_dir/tailscaled.sock")"
tailscaled_bin="$(configured_value "LITEHARNESS_TAILSCALED_BIN" "/usr/local/opt/tailscale/bin/tailscaled")"
