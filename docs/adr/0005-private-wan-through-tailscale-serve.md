...
OpenCozy may support Private WAN Access for Enrolled Devices through Tailscale Serve while keeping direct LAN access available as the vendor-neutral path. The first WAN trajectory keeps OpenCozy as a developer-mode tool: the browser reaches a single HTTPS OpenCozy Origin served through Tailscale Serve, the Vite developer server may remain that origin, and the backend stays localhost-only behind it.
